...
// OpenCL kernel
__kernel void opencl_md5_kernel(__global u32_t *data_storage_device, u32_t v1, u32_t v2) {
    ...
    // Insert complexity using thread ID
    coin[8u] = 0x20202020;
    ...
    // Main loop for processing coins
    for (n = 0u; n < number_coins; n++) {
        ...
        // Gets the current idx if it has encountered a new coin, then increments it with atomic add
        u32_t idx = data_storage_device[0];

        // Check and store the found coin in data_storage_device
        if (hash[3] == 0 && idx < 1024 - 13) {
            idx = atomic_add(&data_storage_device[0], 13); // Atomic index increment
            printf("Coin: %52.52s", (char *)coin);
            for (int j = 0; j < 13; j++) {
                // Store the coin in the data buffer
                data_storage_device[idx + j] = coin[j];
            }
        }

        // Increment the value at index 9 and check overflow for each byte in coin
